...
import (
	...
	"github.com/k3rn31/playground/proglog/internal/config"
	"github.com/k3rn31/playground/proglog/internal/log"
...
func setupTest(t *testing.T, fn func(*Config)) (rootClient, nobodyClient api.LogClient, cfg *Config, teardown func()) {
	...
	newClient := func(ctrPath, keyPath string) (
		*grpc.ClientConn,
		api.LogClient,
		[]grpc.DialOption,
	) {
		tslConfig, err := config.SetupTLSConfig(config.TLSConfig{
			CertFile: ctrPath,
			KeyFile:  keyPath,
		...
		tslCreds := credentials.NewTLS(tslConfig)
		opts := []grpc.DialOption{grpc.WithTransportCredentials(tslCreds)}
